        ...
        for well in row:
            if well in QC_filt.keys():
                ...
                for sweep in range(trace.NofSweeps):
                    ...
                    b_0, b_1 = linear_reg(V[ramp_bounds[0]:ramp_bounds[1]+1], I[ramp_bounds[0]:ramp_bounds[1]+1])
                    I_leak = b_1*V + b_0
...
def plot_leak_fit(currents, QC_filt, well, sweep, ramp_bounds, save=False):
    ...
    ax1.set_xlim(left = ramp_bounds[0]*0.0005 - 1, right = ramp_bounds[1]*0.0005 + 1)
    ...
    ax2.set_xlim(left = ramp_bounds[0]*0.0005 - 1, right = ramp_bounds[1]*0.0005 + 1)
    
    ax3.set_title('current vs voltage')
    ax3.set_xlabel('voltage (mV)')
    ax3.set_ylabel('current (pA)')
    ax3.plot(V[ramp_bounds[0]:ramp_bounds[1]+1], I[ramp_bounds[0]:ramp_bounds[1]+1], 'x')
    ax3.plot(V[ramp_bounds[0]:ramp_bounds[1]+1], I_leak[ramp_bounds[0]:ramp_bounds[1]+1], '--')
    
    ax4.set_title(f'current vs. time (gleak: {np.round(b_1,1)}, Eleak: {np.round(b_0/b_1,1)})')
    ax4.set_xlabel('time (s)')
    ax4.set_ylabel('current (pA)')
    ax4.plot(currents['times'], I, label = 'I_obs')
    ax4.plot(currents['times'], I_leak, linestyle = '--', label = 'I_leak')
    ax4.plot(currents['times'], I - I_leak, linestyle = '--', alpha = 0.5, label = 'Ikr')
    ax4.legend()
